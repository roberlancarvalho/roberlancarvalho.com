...
async function generateRssItem(article: Article): Promise<string> {
  const content = await markdownToHtml(article.content || '')

  return `
    <item>
      <guid>${BLOG_URL}/${article.slug}</guid>
      <title>${article.frontmatter.title}</title>
      <description>${article.frontmatter.description}</description>
      <link>${BLOG_URL}/${article.slug}</link>
...
export async function generateRss(articles: Article[]): Promise<string> {
